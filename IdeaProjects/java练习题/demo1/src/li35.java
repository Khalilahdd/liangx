...
public class li35 {
    ...
    private static void getTwoPrimeNum(int num) {
        List<Integer> primeNumberList = getTwoPrimeNumber(2,num);
     ...
     * @param b
     */
    public static List<Integer> getTwoPrimeNumber(int a,int b){
        List<Integer> primeNuberList = new ArrayList<>();
        if(a >= b){
            System.out.println("a不能大于等于b！");
        }
        for(int i = a;i <= b; i++){
            if(isPrimeNumber(i)){
                primeNuberList.add(i);
            }
        }
        return primeNuberList;
    }
    /**
     * 判断num是否是一个素数
     * @param i
